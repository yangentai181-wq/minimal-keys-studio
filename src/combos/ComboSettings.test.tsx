...
function requestAndConfirmDelete() {
...
describe("ComboSettings save confirmation", () => {
  ...
  it("does not delete a combo until the destructive confirmation is accepted", async () => {
    mocks.subsystem = { callRPC: vi.fn().mockResolvedValue(getAllResponse(true)) };
    renderSettings();
    await waitFor(() => expect(screen.getByRole("button", { name: "編集" })).toBeInTheDocument());
    ...
    expect(screen.getByRole("heading", { name: "コンボを削除" })).toBeInTheDocument();
