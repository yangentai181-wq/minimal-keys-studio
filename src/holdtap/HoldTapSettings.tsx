...
export function HoldTapSettings() {
    ...
    async (label: string, payload: Uint8Array, timeoutMs = 5000) => {
      ...
      try {
        ...
        const data = await Promise.race([
          subsystem.callRPC(payload, timeoutMs),
          timeout,
        ]);
    ...
    async function discover() {
      ...
      try {
        ...
        if (version === discoveryVersionRef.current) {
          console.error("[HoldTap] Discovery failed:", { errorType: isTimeout(e) ? "timeout" : "failure" });
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      if (version === saveVersionRef.current) {
        feedback.clear();
        console.error("[HoldTap] Failed to save:", { errorType: isTimeout(e) ? "timeout" : "failure" });
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      ...
      feedback.clear();
      console.error("[HoldTap] Failed to reset:", { errorType: isTimeout(e) ? "timeout" : "failure" });
  ...
  return (
    <div className="p-4 flex flex-col gap-4 overflow-y-auto max-h-full">
      ...
      >
        <p>長押し（ホールドタップ）の設定を初期値に戻します。現在の設定値は失われ、元に戻せません。続けますか？</p>
      </ConfirmDialog>
    </div>
  );
}
