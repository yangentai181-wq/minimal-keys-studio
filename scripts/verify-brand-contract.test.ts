...
describe("Key Studio repository brand contract", () => {
  ...
  it("rejects a package lock whose release version differs from package.json", () => {
    withFixture((fixture) => {
      const lockPath = join(fixture, "package-lock.json");
      const packageLock = JSON.parse(readFileSync(lockPath, "utf8"));
      packageLock.version = "0.5.0";
      packageLock.packages[""].version = "0.5.0";
      writeFileSync(lockPath, `${JSON.stringify(packageLock, null, 2)}\n`);

      expect(findBrandContractViolations(fixture)).toEqual(expect.arrayContaining([
        "Package lock version: expected 0.6.0, received 0.5.0",
        "Package lock root version: expected 0.6.0, received 0.5.0",
      ]));
    });
  });
