...
| 確認 | 結果 |
| --- | --- |
| `npm test -- --run src/encoder/EncoderSettings.test.tsx` | 終了コード0、1ファイル・5テスト成功 |
| 直後の `npm test` | 終了コード0、115ファイル・733テスト成功 |

- 直前の1件は直後に再現せず、残る回帰ではなく実行順や待機時間に左右される一時的な揺れと確認した。
