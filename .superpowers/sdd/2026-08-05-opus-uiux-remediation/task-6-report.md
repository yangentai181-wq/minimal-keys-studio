...
- Replaced the optional bundle scan with a mandatory `verify:local-fonts` build gate. Every `npm run build` now scans the freshly generated `dist` tree and fails on a Google Fonts URL.
...
- `npm run build` — the fresh-bundle `verify:local-fonts` gate passed.
...
- Scanner fixture verification — a Google Fonts fixture makes the scanner process exit non-zero; a local-only fixture passes.
